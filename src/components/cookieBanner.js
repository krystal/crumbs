export const cookieBanner = `
  <div class="crumbs-banner">
    <div class="crumbs-banner__content">
      <h4 class="crumbs-banner__title">We use cookies</h4>
      <p class="crumbs-banner__description">By clicking “Accept All Cookies”, you agree to the storing of cookies on your device to enhance site navigation, analyze site usage, and assist in our marketing efforts.</p>
    </div>
    <div>
      <button class="crumbs-button crumbs-edit-settings">Edit settings</button>
      <button class="crumbs-button crumbs-accept-all">Accept all cookies</button>
    </div>
  </div>
`;
