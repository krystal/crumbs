...
export default class Crumbs extends EventEmitter {
  constructor({
    ...
    this.types = activeTypes(types);
    this.version = version;

    // Try to load the existing cookie
    this.cookie = this.load();

    console.log("Cookie Value:", this.cookie);
  ...
  outdated() {
    console.log("Cookie Version:", this.cookie.version);
    console.log("Current Cookie Version:", this.version);
  ...
  createTemplate({ identifier, summary, required, title }) {
    console.log(identifier, this.accepted, this.accepted.includes(identifier));
  ...
  */
  ...
  getVersion(value) {
    ...
    const splitVersion = value.split("|");

    console.log("Split Version:", splitVersion);
    console.log("Version Number:", this.getVersionNumber(splitVersion[0]));

    return this.getVersionNumber(splitVersion[0]);
  }
